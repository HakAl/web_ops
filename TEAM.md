...
**Session 18 (2026-09-03)**: Drafted "The Project Directory Is Not Where Your Agent Ends" at `drafts/agent-sandbox-boundary/` (post, claims table with 34 rows, README with team notes). Source: operator's stay-in-project hook plus native sandbox experiment, notes at `~/dev/wip/blog-notes-stay-in-project-2026-09-03.md`. Verified live this session: hook and settings read directly, suite 26/26, Chrome "Failed to create socket directory" reproduced, `MAC_CHROMIUM_TMPDIR` puts the singleton socket in the project, and a new finding (round six): Chrome still dies under the final config at a Mach `bootstrap_check_in` or a sysctl check, so `allowAllUnixSockets` was not enough. Docs and two closed issues (#52471, #70762) confirmed via API. Open gates in the claims table: placeholders for home path and session id, "M4 Pro", `allowUnixSockets` bind semantics (hedged), Chromium 152 line number. Not started: HTML, hero, index, home, sitemap. Tracked in the bead filed this session. Landfall: two Devpost commits were unpushed and went out with this push; six Landfall draft files and `recording-capture-guide.md` were left uncommitted on purpose, they are another session's WIP.
