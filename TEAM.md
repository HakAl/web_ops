...
**Status**: Operational
**Genesis**: 2026-01-31
**Last Session**: 2026-09-03 (session 18): agent sandbox boundary post, rev 1 draft

**Session 18 (2026-09-03)**: Drafted "The Project Directory Is Not Where Your Agent Ends" at `drafts/agent-sandbox-boundary/` (post, claims table with 34 rows, README with team notes). Source: operator's stay-in-project hook plus native sandbox experiment, notes at `~/dev/wip/blog-notes-stay-in-project-2026-09-03.md`. Verified live this session: hook and settings read directly, suite 26/26, Chrome "Failed to create socket directory" reproduced, `MAC_CHROMIUM_TMPDIR` puts the singleton socket in the project, and the reason the global socket switch failed (round six): Chrome dies at a Mach `bootstrap_check_in` or a sysctl check even with `allowAllUnixSockets` on. Operator removed that switch the same day and scoped `excludedCommands` for three npm scripts to the ai-research workspace's local settings (round seven). Rev 2 reflects that. Docs and two closed issues (#52471, #70762) confirmed via API. Open gates in the claims table: placeholders for home path and session id, "M4 Pro", `allowUnixSockets` bind semantics (hedged), Chromium 152 line number. Not started: HTML, hero, index, home, sitemap. Tracked in the bead filed this session. Landfall: two Devpost commits were unpushed and went out with this push; six Landfall draft files and `recording-capture-guide.md` were left uncommitted on purpose, they are another session's WIP.

**Session 16 (2026-04-19)**: Published "An Hour Down Claude Code's Memory Hole" — a service-journalism debugging diary about Claude Code's auto-memory feature consuming roughly 47% (precise: 46.9%) of every system prompt by default. Wire-level data via Langley shows the auto-memory section grew from 1,759 chars on Claude Code 2.1.58 to 12,540 chars on 2.1.114 (7x growth). Three-way comparison captured (default-on 2.1.114, default-on 2.1.58, opt-out 2.1.114) in `langley/exports/`. Post lands on a service-journalism note: turn on `--verbose`, look at the system prompt, flip the switch (`CLAUDE_CODE_DISABLE_AUTO_MEMORY=1`). Also names "act before read" as a model behavior amplified by but independent of the memory feature. Full Blog Release Protocol run (Nora drafted, Dee voice/pattern pass — title broke "I/Our/We"-led pattern, no parenthetical — Blake built HTML + 1600x800 hero image showing the 47% bar, Ari + Grace gates passed). Blog index, home page, sitemap all updated.
**Note for team**: Parapet landing page is NO LONGER in this repo. Canonical location: `parapet/docs/` in the Parapet-Tech/parapet repo. A README pointer is at `docs/parapet/README`. Site is still at **vibecoder.buzz** for the blog. **Rules**: no em dashes in any writing. No `--` (double hyphen) as punctuation either. Both are LLM markers. **dev.to API**: keys are valid. Earlier diagnosis ("account-level issue") was wrong. Cloudflare in front of dev.to bot-blocks the default `Python-urllib/3.X` User-Agent and returns 403 with empty body. Fix: add `'User-Agent': 'curl/8.0.0'` (or any non-default UA) to the headers dict on any urllib/requests call. curl works out of the box because its default UA is whitelisted. Verified with `GET /users/me` returning 200 and `username=theskillsteam` once UA is set.
**What's next**: dev.to key regeneration (priority — blocks automated crossposting). Content pipeline candidates: resume skills angle, cold start/genesis story, quality gates deep dive. Potential follow-up post on "act before read" as its own behavior pattern (named in this post but worth a dedicated piece).
