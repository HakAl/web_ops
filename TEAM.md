...
**Note for team**: Parapet landing page is NO LONGER in this repo. Canonical location: `parapet/docs/` in the Parapet-Tech/parapet repo. A README pointer is at `docs/parapet/README`. Site is still at **vibecoder.buzz** for the blog. **Rules**: no em dashes in any writing. No `--` (double hyphen) as punctuation either. Both are LLM markers. **dev.to API**: all three saved keys in `.keys/devto_key.txt` return 403 on every endpoint (including read-only `GET /users/me`). Account-level issue, not scope-level. Crosspost was attempted automatically and failed; user will need to regenerate keys from dev.to settings before next crosspost, or continue manual posting.
